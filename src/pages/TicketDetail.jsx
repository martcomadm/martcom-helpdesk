...
import { formatDuration, formatSlaTarget, getTicketSla, slaBadge } from '../lib/sla';
...
function targetStatus(target) {
  if (target.actualAt) return target.met ? { label: 'Cumplido', tone: 'met' } : { label: 'Incumplido', tone: 'breached' };
  if (target.breached) return { label: 'Vencido', tone: 'breached' };
  if (target.warning) return { label: 'En riesgo', tone: 'warning' };
  return { label: 'Dentro de SLA', tone: 'ok' };
}
function targetTimeText(target) {
  if (target.actualAt) return `Consumido: ${formatDuration(target.elapsedMs)}`;
  if (target.breached) return `Vencido por: ${formatDuration(Math.abs(target.remainingMs))}`;
  return `Restante: ${formatDuration(target.remainingMs)}`;
}
...
export default function TicketDetail() {
  ...
  const mayEditOperational = (canManage || isMine) && !isLocked;
  ...
  const sla = useMemo(() => ticket ? getTicketSla(ticket) : null, [ticket]);
  const overallSla = useMemo(() => ticket ? slaBadge(ticket) : null, [ticket]);
  ...
  async function takeTicket() {
    ...
    if (isLocked) return setError('Este ticket está finalizado y no puede tomarse.');
    ...
    try { await notifyRequester({ ticket, type: 'assignment', title: `${personName(user)} está atendiendo tu ticket`, message: ticket.folio }); } catch (err) { console.warn(err); }
  }

  async function changeAssignee(value) {
    if (!canManage) return setError('Solo supervisión puede reasignar tickets.');
    if (isLocked) return setError('Reactiva el ticket antes de modificar el responsable.');
  ...
  async function changePriority(value) {
    if (!mayEditOperational) return setError(isLocked ? 'Reactiva el ticket antes de modificarlo.' : 'Debes tomar el ticket antes de modificarlo.');
  ...
  async function changeStatus(value) {
    if (!mayEditOperational) return setError(isLocked ? 'Solo supervisión puede reactivar un ticket finalizado.' : 'Debes tomar el ticket antes de modificarlo.');
  ...
  async function reactivateTicket() {
    ...
    const action = isClosed ? 'reabrió' : 'reactivó';
    await patchTicket({ status: 'en_proceso' }, 'Ticket reactivado correctamente.', [{ message: `${personName(user)} ${action} el ticket. Estado cambiado de ${oldLabel} a En proceso. Se conserva el SLA histórico del ciclo original.`, field: 'status', oldValue: oldStatus, newValue: 'en_proceso' }]);
    try { await notifyRequester({ ticket, type: 'status_change', title: `Ticket reactivado: ${ticket.folio}`, message: `${oldLabel} → En proceso` }); } catch (err) { console.warn(err); }
  }

  async function submitMessage(e) {
    e.preventDefault();
    if (isLocked) return setError('Este ticket está finalizado. Debe reactivarse antes de agregar respuestas o notas.');
  ...
  const responseState = sla ? targetStatus(sla.response) : null;
  const resolutionState = sla ? targetStatus(sla.resolution) : null;
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content">
        ...
        {ticket && <>
          {canWork && <article className="card management-card"><div className="management-head"><div><p className="eyebrow">GESTIÓN IT</p><h2>{isClosed ? 'Ticket cerrado' : isResolved ? 'Ticket resuelto' : 'Atender ticket'}</h2></div>{isLocked ? (canManage && <button onClick={reactivateTicket} disabled={saving}>{isClosed ? 'Reabrir ticket' : 'Reactivar ticket'}</button>) : !ticket.assigned_to && <button onClick={takeTicket} disabled={saving}>Tomar ticket</button>}</div><div className="management-grid"><label>Responsable{canManage ? <select value={ticket.assigned_to || ''} onChange={(e) => changeAssignee(e.target.value)} disabled={saving || isLocked}><option value="">Sin asignar</option>{supportUsers.map((item) => <option key={item.id} value={item.id}>{personName(item)}</option>)}</select> : <input value={ticket.assigned_to ? (isMine ? 'Asignado a ti' : personName(ticket.expand?.assigned_to)) : 'Sin asignar'} disabled />}</label><label>Prioridad<select value={ticket.priority || 'media'} onChange={(e) => changePriority(e.target.value)} disabled={saving || isLocked || (!canManage && !isMine)}><option value="baja">Baja</option><option value="media">Media</option><option value="alta">Alta</option><option value="critica">Crítica</option></select></label><label>Estado<select value={ticket.status} onChange={(e) => changeStatus(e.target.value)} disabled={saving || isLocked || (!canManage && !isMine)}><option value="nuevo">Nuevo</option><option value="en_proceso">En proceso</option><option value="esperando_usuario">Esperando usuario</option><option value="esperando_tercero">Esperando tercero</option><option value="resuelto">Resuelto</option>{canManage && <option value="cerrado">Cerrado</option>}<option value="cancelado">Cancelado</option></select></label></div>{!canManage && !ticket.assigned_to && !isLocked && <p className="muted" style={{ marginTop: 12 }}>Toma el ticket para habilitar prioridad, estado, respuestas y notas internas.</p>}{isLocked && <p className="muted" style={{ marginTop: 12 }}>{canManage ? `El ticket está ${isClosed ? 'cerrado' : 'resuelto'} y bloqueado. Usa “${isClosed ? 'Reabrir ticket' : 'Reactivar ticket'}” para volver a trabajarlo.` : `El ticket está ${isClosed ? 'cerrado' : 'resuelto'}. Solo supervisión puede reactivarlo.`}</p>}</article>}
