...
export default function SupportPanel() {
  ...
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((ticket) => {
      ...
      return matchesSearch && matchesSla && (status === 'todos' || ticket.status === status) && (priority === 'todas' || ticket.priority === priority) && (category === 'todas' || ticket.category === category) && (department === 'todos' || ticket.department === department);
    });
  }, [tickets, usersById, search, status, priority, category, department, slaFilter]);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content">
        ...
        <div className="support-results-head"><span>{filteredTickets.length} ticket{filteredTickets.length === 1 ? '' : 's'}</span><button className="secondary" onClick={() => { setSearch(''); setStatus('todos'); setPriority('todas'); setCategory('todas'); setDepartment('todos'); setSlaFilter('todos'); }}>Limpiar filtros</button></div>
