...
export default function SupportPanel() {
    ...
    async function loadData() {
      try {
        const [ticketRecords, categoryRecords] = await Promise.all([
          pb.collection('hd_tickets').getFullList({
            sort: '-created',
  ...
  const departments = useMemo(() => {
    return [...new Set(tickets.map((ticket) => ticket.department).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  }, [tickets]);

  const filteredTickets = useMemo(() => {
    const term = search.trim().toLowerCase();
    return tickets.filter((ticket) => {
      const requester = ticket.expand?.requester?.name || ticket.expand?.requester?.email || '';
      const categoryName = ticket.expand?.category?.name || '';
      const matchesSearch = !term || [ticket.folio, ticket.title, ticket.description, requester, categoryName, ticket.department, ticket.equipment]
      ...
      const matchesDepartment = department === 'todos' || ticket.department === department;
      return matchesSearch && matchesStatus && matchesPriority && matchesCategory && matchesDepartment;
    });
  }, [tickets, search, status, priority, category, department]);

  const stats = useMemo(() => ({
    nuevos: tickets.filter((ticket) => ticket.status === 'nuevo').length,
    proceso: tickets.filter((ticket) => ticket.status === 'en_proceso').length,
    esperando: tickets.filter((ticket) => ticket.status === 'esperando_usuario' || ticket.status === 'esperando_tercero').length,
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          ...
          <a className="active">Panel de soporte</a>
        </nav>
      ...
      <section className="content">
        <header className="topbar">
          <div>
            <p className="muted">Mesa de ayuda</p>
            <h1>Panel de soporte</h1>
            <p className="muted">Bandeja general de incidencias y solicitudes.</p>
          </div>
          <span className="role-badge">{user?.role}</span>
        </header>

        <div className="stats-grid support-stats">
          <article className="card"><span>Nuevos</span><strong>{stats.nuevos}</strong></article>
          <article className="card"><span>En proceso</span><strong>{stats.proceso}</strong></article>
          <article className="card"><span>Esperando</span><strong>{stats.esperando}</strong></article>
          <article className="card"><span>Sin asignar</span><strong>{stats.sinAsignar}</strong></article>
        </div>
        ...
        <div className="support-results-head">
          ...
          <button className="secondary" onClick={() => { setSearch(''); setStatus('todos'); setPriority('todas'); setCategory('todas'); setDepartment('todos'); }}>Limpiar filtros</button>
        </div>

        {error && <div className="error">{error}</div>}

        {loading ? (
          <article className="card empty-state"><p>Cargando tickets…</p></article>
        ) : filteredTickets.length === 0 ? (
          <article className="card empty-state"><h2>Sin resultados</h2><p>No hay tickets que coincidan con los filtros seleccionados.</p></article>
        ) : (
          <div className="support-table-wrap card">
              ...
              <tbody>
                {filteredTickets.map((ticket) => (
                  <tr key={ticket.id} onClick={() => navigate(`/tickets/${ticket.id}`)}>
                    <td><strong>{ticket.folio}</strong><span className="table-subtext">{ticket.title}</span></td>
                    <td>{ticket.expand?.requester?.name || ticket.expand?.requester?.email || '—'}</td>
                    <td>{ticket.department || '—'}</td>
                    <td>{ticket.expand?.category?.name || '—'}</td>
                    <td><strong className={`priority-${ticket.priority}`}>{ticket.priority || '—'}</strong></td>
                    <td><span className={`status-badge status-${ticket.status}`}>{statusLabels[ticket.status] || ticket.status}</span></td>
                    <td>{ticket.expand?.assigned_to?.name || ticket.expand?.assigned_to?.email || 'Sin asignar'}</td>
