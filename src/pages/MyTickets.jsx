...
export default function MyTickets() {
  ...
  useEffect(() => {
    async function loadTickets() {
      try {
        const records = await pb.collection('hd_tickets').getFullList({
  ...
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((ticket) => {
      ...
      const matchesSearch = !term || [ticket.folio, ticket.title, ticket.description, category, ticket.equipment]
      ...
      <section className="content">
        <header className="topbar">
          <div>
            ...
            <h1>Mis tickets</h1>
            <p className="muted">Consulta el estado de las solicitudes que has registrado.</p>
          </div>
        ...
        <div className="ticket-toolbar">
          <input
            ...
            placeholder="Buscar por folio, asunto, categoría o equipo…"
          />
        ...
        {loading ? (
          <article className="card empty-state"><p>Cargando tus tickets…</p></article>
        ) : filteredTickets.length === 0 ? (
          <article className="card empty-state">
            <h2>{tickets.length ? 'Sin coincidencias' : 'Aún no tienes tickets'}</h2>
            <p>{tickets.length ? 'Prueba con otro texto o estado.' : 'Cuando registres una solicitud aparecerá aquí.'}</p>
            {!tickets.length && <button onClick={() => navigate('/tickets/new')}>Crear mi primer ticket</button>}
          </article>
        ) : (
              ...
              <article className="card ticket-row" key={ticket.id}>
                <div className="ticket-main">
                  ...
                  <div className="ticket-details">
                    <span>Categoría: <strong>{ticket.expand?.category?.name || '—'}</strong></span>
                    <span>Prioridad: <strong className={`priority-${ticket.priority}`}>{ticket.priority || '—'}</strong></span>
                    <span>Equipo: <strong>{ticket.equipment || '—'}</strong></span>
                    <span>Creado: <strong>{formatDate(ticket.created)}</strong></span>
                  </div>
                </div>
              </article>
            ))}
